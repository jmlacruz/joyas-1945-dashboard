...
import { Pano, panoFieldsFromDB } from '../../../../types/DASHBOARD/database';
import { getTable, updateTable } from '../../../../services/database';
import { showModal1 } from '../../../../features/modalSlice';
import { clothFormRequiredFields } from '../../../../data';
...
const ClothPage = () => {
	...
	const [userData, setUserData] = useState <Pano | null> (null);
	const initialData = useRef <Pano | object> ({});
		
	const formik: FormikProps <Pano> = useFormik({
		initialValues: {
			id: 0,
			nombre: "",
		},
		onSubmit: async (values: Pano) => {	
			showSpinner(true);

			const emptyRequiredFieldsParsedArr: string[] = [];										//Verifica si hay campos obligatorios vacíos
			clothFormRequiredFields.forEach((requiredField) => {
			...
			const response = await updateTable({tableName: "pano", conditions: [{field: "id", value: idParsed}], data: formik.values});
			if (response.success) {
	...
	useEffect(() => {
		(async () => {
			showSpinner(true);
			const response2 = await getTable ({tableName: "pano", conditions: [{field: "id", value: idParsed}], fields: panoFieldsFromDB});
			if (response2.success && response2.data && response2.data.length) {
				const data: Pano = response2.data[0];
				setUserData(data);
			}
			showSpinner(false);
		})();
	...
	return (
		<PageWrapper name="Edición de paño">
			<Subheader>
				<SubheaderLeft>
