...
import {
	DocumentsInfoOfNewBrand,
	ImagesInfoOfNewBrand,
	Marca,
	NewBrandDocumentsInfo,
	NewBrandImagesInfo,
...
const BrandNewPage = () => {
	...
	const formik: FormikProps<Partial<Marca>> = useFormik({
		...
		onSubmit: async (values: Partial<Marca>) => {
			...
			if (brandImages.frontPage.file || brandImages.logo.file) {
				...
				if (brandImages.frontPage.file)
					imagesData.append('files', brandImages.frontPage.file);
					...
					if (response.success && response.data) {
						...
						dispatch(
							showModal1({
								...
								info: {
									icon: 'error',
									title: 'Error al subir PDF Costo',
									subtitle: response.message,
								},
							}),
						);
					...
					if (response.success && response.data) {
						...
						dispatch(
							showModal1({
								...
								info: {
									icon: 'error',
									title: 'Error al subir PDF Recomendado',
									subtitle: response.message,
								},
							}),
						);
				...
				dispatch(
					showModal1({
						...
						info: {
							...
							subtitle: 'Marca actualizada correctamente',
						},
					}),
				);
	...
	const handleInputDocumentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		try {
			...
			dispatch(
				showModal1({
					show: true,
					info: { icon: 'error', title: 'Error al cargar documento', subtitle: message },
				}),
			);
		}
	};
	...
	const deleteDocumentInput = (documentType: keyof NewBrandDocumentsInfo) => {
		if (documentType === 'pdf') {
			...
			const recommended_PDFInput = document.querySelector(
				'.recommendedPDFInput',
			) as HTMLInputElement;
						...
						<div className='grid grid-cols-12 gap-4'>
							<div className='col-span-12'>
								<Card>
											...
											<CardTitle>
												<div>
													...
													<div className='text-base font-normal text-zinc-500'>
														...
														<span className='requiredFieldSymbol'>
															*
														</span>{' '}
														son obligatorios
													</div>
									...
									<CardBody>
										<div className='formFieldsMarginBottom grid grid-cols-12 gap-4'>
									...
									<CardHeader className='min-h-20 content-between'>
										...
										{brandImages.frontPage.strB64 && (
											<Button1
												color='red'
												variant='solid'
												onClick={() => deleteImageInput('frontPage')}>
												Eliminar
											</Button1>
										)}
									</CardHeader>
										...
										<div
												...
												<div className='mt-4 flex text-sm leading-6 text-gray-500 '>
													<label
														...
														<input
															...
															name='frontPage'
															type='file'
															className='frontPageInput sr-only'
											...
											<div className='dflex column h-full w-1/2 border border-dashed border-zinc-500/25 text-center'>
												<WaitImages>
													{brandImages.frontPage.strB64 && (
														<img
															src={brandImages.frontPage.strB64}
															alt=''
								...
								<Card>
									<CardHeader className='min-h-20 content-between'>
										...
										{brandDocuments.pdf.file && (
											<Button1
												color='red'
												variant='solid'
												onClick={() => deleteDocumentInput('pdf')}>
												Eliminar
											</Button1>
										)}
									</CardHeader>
									<CardBody>
										...
										<div
											...
											<div className='dflex column min-h-32 w-1/2 border border-dashed border-zinc-500/25 text-center'>
												<Icon
												...
												<div className='mt-4 flex text-sm leading-6 text-gray-500 '>
													<label
														...
														<input
															...
															name='pdf'
															type='file'
															className='brandPDFfInput sr-only'
											...
											<div className='dflex column min-h-32 w-1/2 border border-dashed border-zinc-500/25 text-center'>
												{brandDocuments.pdf.file && (
													<p className='w-full break-words object-contain px-8'>
														{' '}
														{brandDocuments.pdf.file?.name}{' '}
													</p>
												)}
											</div>
										</div>
							...
							<div className='col-span-12 lg:col-span-6'>
								<Card>
									<CardHeader className='min-h-20 content-between'>
										<CardHeaderChild>
											<CardTitle className='text-xl'>
												{brandDocuments.recommended_pdf.file
													? 'Cambiar PDF Recomendado'
													: 'Subir PDF Recomendado'}
											</CardTitle>
										</CardHeaderChild>
										{brandDocuments.recommended_pdf.file && (
											<Button1
												...
												variant='solid'
												onClick={() =>
													deleteDocumentInput('recommended_pdf')
												}>
												Eliminar
											</Button1>
										)}
												...
												<div className='mt-4 flex text-sm leading-6 text-gray-500 '>
													<label
														...
														<input
															...
															type='file'
															className='recommendedPDFInput sr-only'
															onChange={handleInputDocumentChange}
															accept='.pdf, .doc, .docx, .txt'
															multiple={false}
														/>
