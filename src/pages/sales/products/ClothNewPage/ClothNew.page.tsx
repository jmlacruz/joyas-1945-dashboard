...
const formikInitialValues: Partial<Pano> =  {
	nombre: "",
};
...
const ClothNewPage = () => {
	...
	const formik: FormikProps <Partial<Pano>> = useFormik({
		initialValues: formikInitialValues,
		onSubmit: async (values: Partial<Pano>) => {
			showSpinner(true);
			...
			const response = await insertRow({tableName: "pano", data: formik.values});
			if (response.success) {
				formik.resetForm({
	...
	return (
		<PageWrapper name="Creación de paño">
			<Subheader>
				<SubheaderLeft>
					<Link
						to={`../${appPages.salesAppPages.subPages.productPage.subPages.ClothsListPage.to}`}>
