...
import "./pricesGroupNew.page.css";
...
import { GrupoFormik } from '../../../../types/DASHBOARD/database';

import { SpinnerContext } from '../../../../context/spinnerContext';
import { insertRow } from '../../../../services/database';
import { pricesGroupFormFormikRequiredFields } from "../../../../data";
import useSaveBtn from "../../../../hooks/useSaveBtn";

const formikInitialValues: GrupoFormik =  {
	nombre: "",
	valor: "",
};

const PricesGroupNewPage = () => {

	const {showSpinner} = useContext(SpinnerContext);
	const dispatch = useDispatch();

	const formik: FormikProps <GrupoFormik> = useFormik({
		initialValues: formikInitialValues,
		onSubmit: async (values: GrupoFormik) => {
			showSpinner(true);

			const emptyRequiredFieldsParsedArr: string[] = [];										//Verifica si hay campos obligatorios vacíos
			pricesGroupFormFormikRequiredFields.forEach((requiredField) => {
			...
			const response = await insertRow({tableName: "grupo", data: formik.values});
			if (response.success) {
				formik.resetForm({
	...
	return (
		<PageWrapper>
			<Subheader>
				<SubheaderLeft>
					<Link
						to={`../${appPages.salesAppPages.subPages.productPage.subPages.pricesGroupsListPage.to}`}>
							...
							<Card>
								<CardHeader>
									<CardHeaderChild>
										<CardTitle>
											<div>
												<div>Creación de Grupo de precios</div>
...
export default PricesGroupNewPage;
